...
There are different types of R Markdown documents. We will use the R Notebook the most frequently
...
```{r r-notebook, echo = F, fig.cap = 'How to create a new R Notebook', fig.width = 6, out.width = "60%"}
...
```{block, type='rmdinfo'}
Note that there is now a R Notebook file (`.Rmd`) and an html file (`nb.html`) in the `notebook` folder.
```
