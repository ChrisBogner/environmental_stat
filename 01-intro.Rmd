...
R is a programming language for data analysis and statistical modelling. It is free (*open-source software*) and belongs together with Python to the most used programming languages for data analysis. R has been introduced by Ross Ihaka and Robert Gentleman in 1996 [@Ihaka1996]. It has many additional *packages*, which extend its functionality.

You can and should install R on your computer through the official [R webpage](https://www.r-project.org/). A short installation HowTo can be found on ILIAS and in @ModernDive, Chapter 1. You will also find additional packages on R's official webpage under CRAN (*The Comprehensive R Archive Network*). Not all packages are released through CRAN; however, in the beginning it is a good idea to use CRAN to find and install packages. 

Packages can be organized in topics, which you can explore via the [CRAN Task Views](http://cran.r-project.org/web/views/). For environmental statistics, the following topics are relevant:
...
On the top left, you will type your commands and your text. The focus of this course is on reproducible research, and we will start using RMarkdown in the next session.
